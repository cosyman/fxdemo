...
public class PlayingVideo extends Application {
    ...
    private Node createCloseButton(final Scene scene) {
        // close button
    ...
    private void exitApp(Window parent) {
        ...
        // if not run from sampler app
        if (!Boolean.parseBoolean(System.getProperty("sampler.mode"))) {
            Platform.exit();
        }
        // if is run from sampler app
        ((Stage)parent).close();
    }
}